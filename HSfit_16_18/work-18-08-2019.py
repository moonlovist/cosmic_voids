...
r_ori, chi_ori, chi_var = np.loadtxt("/hpcstorage/zhaoc/share/ting/2PCF_disjoint_voids/correlation_function_auto_disjoint_16/average.dat", usecols=(0,1,2), unpack=True)
...
kd_PACHY, Pkd_PACHY, Variance_1_PACHY = np.loadtxt("/hpcstorage/zhaoc/share/ting/PATCHY_pre_voids/powspec_PACHY_dis_voids_16/catelogues/average.dat", usecols=(0,1,2), unpack=True)
test = np.loadtxt("/hpcstorage/zhaoc/share/ting/PATCHY_pre_voids/powspec_PACHY_dis_voids_16/catelogues/average.dat", usecols=(0,1,2), unpack=True)
...
#plt.plot(k_test,P_test, label = "fitting from Hard sphere model - 1")
#plt.plot(k_lin,P_lin, label = "power - spectrum - lin_nw")
#plt.plot(kb,Pb, label = "fitting from Hard sphere model - 2")
#plt.errorbar(kd,Pkd, yerr=Variance_1, label = "power spectrum of voids from Template of radius bins 16 - 18 Mpc")
#plt.fill_between(kd, Pkd - Variance_1, Pkd + Variance_1, color='red', alpha=0.2)

#plt.errorbar(kd_PACHY,Pkd_PACHY, yerr=Variance_1_PACHY, label = "power spectrum of voids from PATCHY of radius bins 16 - 18 Mpc")
...
plt.savefig("/home/epfl/tan/first-code-document/analysis_cos/output/Best - fitting - 16_18 - 6")
