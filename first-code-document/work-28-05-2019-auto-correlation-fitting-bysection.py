...
#get the roots
def LL1(e):
    return (1+e/2.0)/(1.0+2*e)#e = np.pi * rho * d ** 3 / 6.0
def SS1(e):
...
def g(x, e, c, d, sec1, sec2, sec3, sec4):
    k = []
    for r in x/d:
        k.append(-1/(12.0*e*r) *(1/(SS3(e)) *phi1(r-1,LL1(e),ss1(SS1(e),SS2(e),SS3(e)),ss2(SS1(e),SS2(e),SS3(e)),ss3(SS1(e),SS2(e),SS3(e)))*np.heaviside(r-1,0)*sec1+
                        1/(SS3(e)**2) *phi2(r-2,LL1(e),ss1(SS1(e),SS2(e),SS3(e)),ss2(SS1(e),SS2(e),SS3(e)),ss3(SS1(e),SS2(e),SS3(e)))*np.heaviside(r-2,0)*sec2+
                        1/(SS3(e)**3) *phi3(r-3,LL1(e),ss1(SS1(e),SS2(e),SS3(e)),ss2(SS1(e),SS2(e),SS3(e)),ss3(SS1(e),SS2(e),SS3(e)))*np.heaviside(r-3,0)*sec3+
                        1/(SS3(e)**4) *phi4(r-4,LL1(e),ss1(SS1(e),SS2(e),SS3(e)),ss2(SS1(e),SS2(e),SS3(e)),ss3(SS1(e),SS2(e),SS3(e)))*np.heaviside(r-4,0)*sec4))
...
fit_function = interp1d(np.array(file[0]), np.array(file[1]), kind='cubic')
print(fit_function)
# fit code
"""
def C(x,y,lam1,lam2):
...
popt, pcov = curve_fit(g,testx, testy, p0 = [0.3,0.3,42.0,1.0,1.0,1.0,1.0])
print (popt)
#print (g(testx,0.3,0.3,42.0))
plt.figure()
print(file[0])
...
#plt.errorbar(np.array(file[0]), np.real(np.array(file[1])), yerr=np.array(file[2]), label = "voids auto-correlation function")
...
plt.errorbar(np.array(file_cross[0])/dia, np.real(np.array(file_cross[1])), yerr=np.array(file_cross[2]), label = "voids-halos cross-correlation function")
plt.fill_between(np.array(file_cross[0])/dia, np.real(np.array(file_cross[1])) - np.array(file_cross[2]), np.real(np.array(file_cross[1]))+np.array(file_cross[2]), color='red', alpha=0.2)
"""
#print (file[1])
#print (file[2])
#l = 0.3
...
x = np.linspace(0.001,np.max(file[0]))
h = np.real(g(x, popt[0], popt[1], popt[2], popt[3], popt[4], popt[5], popt[6]))
i = 0
for rr in x:
    if rr/popt[2] < 1:
        h[i] = -1
...
plt.plot(np.array(file[0]),fit_function(np.array(file[0])), label ="fitting function")
...
plt.savefig("fitting by section voids auto correlation function - 2")
plt.show()