...
def f2(SS,t):
    return (L(t)/SS)**2
def f2_1or(SS,S1or,t):
    return (2*L(t)*L_1or(t)*SS**2 - L(t)**2 *2*SS*S1or)/(SS**4)
def f3(r,t):
    return np.exp(t*(r - 2*R))
def f3_1or(r,t):
    return (r - 2*R)*np.exp(t*(r - 2*R))

def tfunction2(SS,S1or,r, t):
    return f2(SS,t)*f3(r,t) + t*f2_1or(SS,S1or,t)*f3(r,t) + t*f2(SS,t)*f3_1or(r,t)
#function2 = lambda r, t, ti: (t - ti)**2 *t*(L(t)/S(t))**2 *np.exp(t*(r-2*R))
#tfunction2(r, t, ti) = diff(function2, t, 1)

def sum2(r):
    return tfunction2(S_1(t1),S_1_1or(t1),r, t1) + tfunction2(S_2(t2),S_2_1or(t2),r, t2) + tfunction2(S_1(t1),S_1_1or(t1),r, t3)
def g2(r):
...
k.pop(1)
plt.plot(r[:-1]/R,k[:-1], label ="correlation function")
#print (S_1or(r))
#plt.plot(r,S(r), label ="S")
#plt.plot(r,S_1or(r), label ="S'")
plt.legend(loc="upper right")
plt.savefig("hard_sphere_model-14.png")
plt.show()
