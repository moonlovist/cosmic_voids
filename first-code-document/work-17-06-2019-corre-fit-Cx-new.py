...
def cxfunc(par, x, c_fraction):
  ...
  idx = (x <= d)
  x0 = x[idx] / d
  c[idx] = (lambda1 + 6 * y * lambda2 * x0 + 0.5 * y * lambda1 * x0**3) *c_fraction
  return -c
...
def func_fit(xdata,d,rho,dia,c_fraction):#
  x = xdata/dia
  N = 100000
  kmax = 1e6
  kmin = 1e-6
...
popt, pcov = curve_fit(func_fit,testx, testy, p0 = [1,0.19900193*6/np.pi,40,1.0],  maxfev = 2000) #
print (popt)
...
xi = func_fit(x,popt[0],popt[1],popt[2],popt[3])#
i = 0
for rr in x:
    if rr/popt[2] < 1:
        xi[i] = -1
...
np.savetxt("/home/epfl/tan/first-code-document/fitting correlation function/fitting_correlation_function_disjoint_void_average_1.txt", np.transpose([x,xi]), fmt = '%g')
...
plt.savefig("/home/epfl/tan/first-code-document/fitting correlation function/Cx - 1")
plt.show()
