...
def cxfunc(par, x, C_fraction_0, C_fraction_1, C_fraction_2, C_fraction_3, C_fraction_4):
  d, rho = par[0], par[1]
  c = np.zeros_like(x)
  ...
  idx = (x <= d)
  x0 = x[idx] / d
  c[idx] = lambda1*C_fraction_0 + 6 * y * lambda2 * x0*C_fraction_1 + x0**2 *C_fraction_2 + 0.5 * y * lambda1 * x0**3 * C_fraction_3 + x0**4 * C_fraction_4
  return -c
...
def func_fit(kdata,d,rho,rescale_x,rescale_y,C_fraction_0,C_fraction_1,C_fraction_2,C_fraction_3,C_fraction_4):
  N = 100000
  kmax = 1e6
  #kmin = 1e-6
  kmin = 0.004
  ...
  rmax = 1e6
  y = np.pi * rho * d ** 3 / 6.0
  fcx = lambda x : cxfunc([d, rho], x, C_fraction_0, C_fraction_1, C_fraction_2, C_fraction_3, C_fraction_4)
...
testx = []
testy = []
for x in file3[0][40:125]:
    testx.append(float(x))
testx = np.array(testx)
for y in file3[1][40:125]:
    testy.append(float(y))
testy = np.array(testy/np.max(np.array(file3[1])))
"""

popt, pcov = curve_fit(func_fit,testx,testy, p0 = [1,0.19900193*6/np.pi,38.5,1.5,1.0,1.0,0,1.0,0],  maxfev = 20000)
print (popt)
...
plt.plot(file3[0],func_fit(file3[0],popt[0],popt[1],popt[2],popt[3],popt[4],popt[5],popt[6],popt[7],popt[8]), label = "power - spectrum - auto - correlation - mode5")
...
plt.savefig("Correction - fitting - power - spectrum - 18")
plt.show()
