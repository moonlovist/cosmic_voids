...
#  np.min(file_2pcf[0])  np.max(file_2pcf[0])   1/np.max(file_2pcf[0])
  ...
  return r, xi, Pk, file1, k, ck, Pk1, hk, kk, pkk, PK_correlation_k, PK_correlation_ck
...
r,xi,Pk,xdata,k,ck,Pk1,hk,kk,pkk,PK_correlation_k,PK_correlation_ck= func_fit(1,0.05*6/np.pi)
...
plt.savefig("Correction - fitting - power - spectrum - 8")
#plt.savefig("Fitting - power - spectrum - 14")
plt.show()
