...
def cxfunc(par, x, C_fraction):
  ...
  idx = (x <= d)
  x0 = x[idx] / d
  c[idx] = (lambda1 + 6 * y * lambda2 * x0 + x0**2 + 0.5 * y * lambda1 * x0**3)* C_fraction
  return -c
...
def func_fit(kdata,d,rho,rescale_x,rescale_y, C_fraction):
  N = 100000
  kmax = 1e6
  #kmin = 1e-6
  kmin = 0.004
  ...
  rmax = 1e6
  y = np.pi * rho * d ** 3 / 6.0
  fcx = lambda x : cxfunc([d, rho], x, C_fraction)
...
popt, pcov = curve_fit(func_fit,testx,testy, p0 = [1,0.19900193*6/np.pi,38.5,1.5,1.0],  maxfev = 20000)
print (popt)
...
plt.plot(file3[0],func_fit(file3[0],popt[0],popt[1],popt[2],popt[3],popt[4]), label = "power - spectrum - auto - correlation - mode5")
...
plt.xscale("log")
#plt.yscale("log")
np.savetxt("/home/epfl/tan/first-code-document/fitting_power_spectrum_disjoint_void_average_5.txt", np.transpose([file3[0],func_fit(file3[0],popt[0],popt[1],popt[2],popt[3],popt[4])]), fmt = '%g')
#file_fitting_data = pd.DataFrame(np.loadtxt("fitting_correlation_function_disjoint_void_average.txt"))
#plt.plot(file_fitting_data[0],file_fitting_data[1], label = "fitting - data")
#print(file_fitting_data[0],file_fitting_data[1])
plt.savefig("Correction - fitting - power - spectrum - Cx - 1")
plt.show()
